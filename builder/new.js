var CHANGED = false, PREGOODS = [], DSFRMS, IDX = -2, delConfirmMsg = "建议您先将数据导出后再删除字段！\n删除将导致已提交的数据和报表中此字段对应的值被清空，且不可恢复。确认删除吗？", DEFFLD = {
	...
	item_checkbox : '<li><input name="CHKED" value="1" class="checkbox" type="checkbox" title="默认选中此项" /><input name="VAL" type="text" class="sl"/><a class="icononly-add iconfont" title="添加一个新的选择项">&#xe60b;</a><a class="icononly-del iconfont" title="删除此选择项">&#xe619;</a><a class="icononly-mov iconfont" title="排序">&#xe63d;</a><a class="icononly-limit iconfont" title="添加提交次数限制">&#xe632;</a></li>',
	...
	item_radio : '<li><input name="CHKED" value="1" class="radio" type="radio" title="默认选中此择项" /><input name="VAL" type="text" class="sl"/><a class="icononly-add iconfont" title="添加一个新的选择项">&#xe60b;</a><a class="icononly-del iconfont" title="删除此选择项">&#xe619;</a><a class="icononly-mov iconfont" title="排序">&#xe63d;</a><a class="icononly-limit iconfont" title="添加提交次数限制">&#xe632;</a></li>',
	item_dropdown2 : '<li><input name="VAL" type="text" class="xl"/><a class="icononly-add iconfont" title="添加一个新的选择项">&#xe60b;</a><a class="icononly-del iconfont" title="删除此选择项">&#xe619;</a></li>',
...
function showProperties(c) {
	...
	a = {
		text : ["ptype", "pfldsize", "poptions", "popt_required", "popt_unique", "popt_qrinput", "psecurity", "prange", "pdefval_text", "pinstruct", "pconfine", "pconfine1", "pconfine2"],
		...
		dropdown : ["ptype", "pfldsize", "poptions", "popt_required", "psecurity", "pitems", "pitems_radio", "pinstruct", "pconfine", "pconfine1", "pconfine2"],
		dropdown2 : ["ptype", "poptions", "popt_required", "psecurity", "pitems", "pitems_batchedit", "pitems2", "pinstruct", "pN"],
		...
		phone : ["ptype", "poptions", "popt_required", "popt_unique", "popt_authcode", "psecurity", "pdefval_phone_mobile", "pphoneformat", "pinstruct", "pconfine", "pconfine1", "pconfine2"],
...
function propertyInit() {
	...
	$("#itemList>li>a.icononly-limit").live({
		click : function () {
			...
			if (V.length == 0) {
				var Z = "lmt" + X;
				var Y = '<div id="' + Z + '" class="hide" style="margin-top:5px;"><select class="s" style="margin-left:18px;" name="COMMITLMT"><option value="DAY">每天</option><option value="ALL">累计</option></select> 限制填写 <input class="s number" type="text" name="AMOUNTLMT" maxlength="10"/> 次</div>';
				V = $(Y);
				U.append(V)
			}
			if (V.hasClass("hide")) {
	...
	$("#itemList>li>div>select, #goodsList>li div[name='goods-limit']>select").live({
		change : function () {
			var aa = $(this),
			W = aa.parent();
			var ab;
			...
			Y.find("li>div, li div[name='goods-limit']").find("select[name='COMMITLMT']>option[value='" + F[IDX].COMMITLMT + "']").prop("selected", true)
		}
	});
	$("#itemList>li>div>input, #goodsList>li div[name='goods-limit']>input").live({
		change : function () {
			var U = $(this),
			X = U.parent();
			var ab;
	...
	var d = function (W) {
		if ($(W).parent().hasClass("dropReq")) {
			$(W).parent().find(":radio,:checkbox").prop("checked", false).end().hide();
			span = $("span:last", "#f" + IDX).find(":radio,:checkbox").prop("checked", false).end().hide();
			$("#allowOther").prop("checked", false);
			return
		}
		var V = $(W).parent().index(),
		X = $(W).parent().parent();
		$(W).parent().remove();
		$("#f" + IDX).find("div.content>span:eq(" + V + ")").remove();
		$("#f" + IDX).find("div.content>div.goods-item:eq(" + V + ")").remove();
		...
		if ("itemList2" == X.attr("id")) {
			var U = parseInt(X.attr("IDX"));
			F[IDX].ITMS[U].ITMS.splice(V, 1)
		} else {
			F[IDX].ITMS.splice(V, 1);
	...
	var D = function () {
		...
		X = U.parent(),
		V = U.index();
		if ((X.find("li.dropReq").length == 0 && X.find(">li").length <= 1) || (X.find("li.dropReq").length > 0 && X.find(">li").length <= 2)) {
			$.alert("至少要保留一个选项。");
			return false
		}
		if (F[IDX].ITMS[V].NM || F[IDX].ITMS[V].ITMID && M.MXID > 0) {
			$.confirm({
				msg : delConfirmMsg,
				showyes : true,
				confirm : function () {
					...
					if (F[IDX].ITMS[V].NM) {
						...
						if (F[IDX].ITMS[V].IMG) {
							Z.IMG = F[IDX].ITMS[V].IMG
						}
					} else {
						Z = {
							_id : M._id,
							FLDID : F[IDX].FLDID,
							ITMID : F[IDX].ITMS[V].ITMID,
							TYP : F[IDX].TYP
						}
					}
					$.postJSON(Y, Z, function () {
						d(W);
						$.hideStatus()
					})
				}
	...
	$("#itemList").find("a.icononly-add").live({
		click : function () {
			...
			$(this).parent().after(X);
			var W = $("#f" + IDX).find("div.content>span:eq(" + V + ")");
			W.after($(W).clone().find(":radio,:checkbox").prop("checked", false).end());
	...
	var L = function () {
		var U = $.trim($(this).val());
		$("#f" + IDX).find("div.content").html($.enterToBr(U));
		if (U) {
			F[IDX].SECDESC = U
		} else {
	...
	var i = function () {
		var U = $.trim($(this).val());
		$("#f" + IDX).find("div.content").html($.encodeScript(U));
		if (U) {
			F[IDX].HTML = U
		} else {
			delete F[IDX]["HTML"]
	...
	$.postJSON(b, c, function (f) {
		...
		if (d) {
			$.lightBox({
				...
				loaded : function () {
					var g = $("#divActions");
					g.find("a.preview").attr("href", $.format("/web/formview/{0}", M._id));
					g.find("a.setting").attr("href", $.format("/app/basicsetting/{0}?menuid=m01", M._id));
					g.find("a.rule").attr("href", $.format("/app/formrule/{0}?type=field&menuid=m21", M._id));
					g.find("a.notification").attr("href", $.format("/app/notification/{0}?type=weixin&menuid=m11", M._id));
					g.find("a.relation").attr("href", $.format("/app/fieldrelation/{0}?menuid=m32", M._id));
					g.find("a.publish").attr("href", $.format("/app/formcode/{0}?menuid=m02", M._id));
					$("#btnPublish").attr("href", $.format("/app/formcode/{0}?menuid=m02", M._id));
					g.removeClass("hide")
				}
			})
		}
...
function createField(p, o) {
	...
	h,
	i;
	p.removeClass("one two three oneByOne fieldInstruct").addClass("field default");
	p.attr("title", "点击编辑，拖动排序");
	p.empty();
	$("#nofields").hide();
	$("#formButtons").show();
	h = $(DEFFLD[o.TYP].html);
	if ("goods" == o.TYP && "1" == o.NOIMG) {
		h = $(DEFFLD.goodsnoimg.html)
	}
	if (o.TYP === "html" || o.TYP === "section") {
		m = h.find("label.desc");
		i = h.find("div.content")
	} else {
		m = h.filter("label.desc");
		i = h.filter("div.content")
	}
	if (o.TYP === "likert") {
		m = i.find("label.desc")
	}
	...
	m.text(o.LBL);
	m.append(l);
	if (o.TYP === "text" && "1" == o.QRINPUT) {
		i.find("i.qrinput").removeClass("hide")
	} else {
		if (o.TYP === "phone" && o.FMT) {
			i.html(DEFFLD[$.format("phone_{0}_{1}", o.FMT, M.LANG)]);
			"1" == o.AUTH ? i.find(".sendcode").show() : i.find(".sendcode").hide();
			"1" == o.AUTH ? $("#signcnt").show() : $("#signcnt").hide()
		} else {
			if (o.TYP === "date" && o.FMT) {
				i.html(DEFFLD[$.format("date_{0}", o.FMT)])
			} else {
				if (o.TYP === "name" && o.FMT) {
					if (o.FMT === "short") {
						i.html(DEFFLD.name_short)
					} else {
						i.html(DEFFLD[$.format("name_{0}_{1}", o.FMT, M.LANG)])
					}
				} else {
					if (o.TYP === "address") {
						i.html(DEFFLD[$.format("address_{0}", M.LANG)]);
						if (o.DEF) {
							var f = o.DEF.split("-");
							i.find("select.province").empty().append($.format("<option>{0}</option>", f[0] || "省/自治区/直辖市"));
							i.find("select.city").empty().append($.format("<option>{0}</option>", f[1] || "市"));
							i.find("select.zip").empty().append($.format("<option>{0}</option>", f[2] || "区/县"))
						}
					} else {
						if (o.TYP === "radio") {
							createRadioItemsPreview(o, i)
						} else {
							if (o.TYP === "checkbox") {
								i.empty();
								var a;
								$.each(o.ITMS, function (j, c) {
								...
								if (o.TYP === "image") {
									i.find("img").attr("src", o.IMG ? IMAGESURL + o.IMG : "/rs/images/defaultimg.png")
								} else {
									if (o.TYP === "goods") {
										createGoodsItemsPreView(o, i)
									} else {
										if (o.TYP === "section") {
											i.html($.enterToBr((o.SECDESC || "")))
										} else {
											if (o.TYP === "html") {
												i.html($.encodeScript(o.HTML || ""))
											} else {
												if (o.TYP === "likert") {
													createLikertPreview(o, h)
												} else {
													if (o.TYP === "dropdown2") {
														var b = o.pN || "2";
														if (b !== "2") {
															...
															for (var e = 0; e < b; e++) {
																i.append('<select disabled="disabled" class="input"></select>')
															}
															i.find("select").css({
																width : (100 / b - 1) + "%",
																"margin-right" : "1%"
														...
														for (var d = 0; d < o.ITMS.length; d++) {
															if (o.ITMS[d].CHKED === "1") {
																i.find("select:first").empty().append($.format("<option>{0}</option>", o.ITMS[d].VAL));
																break
															}
														}
													}
	...
	if (o.TYP === "dropdown") {
		$.each(o.ITMS, function (j, c) {
			if (c.CHKED === "1" && c.VAL) {
				i.find("select").append($.tmpl("<option>${$data}</option>", c.VAL));
				return false
			}
		})
	}
	if (o.DEF) {
		var q = ["text", "textarea", "number", "ulr", "email", "money", "phone"];
		if ($.inArray(o.TYP, q) >= 0) {
			if (o.TYP === "phone" && o.FMT === "tel") {
				$.each(o.DEF.split("-"), function (j, c) {
					i.find(":text:eq(" + j + ")").val(c)
				})
			} else {
				h.find(":input").val(o.DEF)
			}
		}
	}
	if (o.FLDSZ) {
		i.find(":text,textarea,select").removeClass("s m xxl").addClass(o.FLDSZ)
	}
	var g = $(DEFFLD.instruct);
	if (o.INSTR) {
		g.text(o.INSTR)
	}
	p.append(DEFFLD.icon).append(h).append(g).append(DEFFLD.fieldActions);
	if (isInstruct(o.TYP)) {
		p.addClass("fieldInstruct")
	}
	if (o.LAY) {
		p.addClass(o.LAY)
	}
...
function formInit() {
	...
	if (M._id) {
		...
		c.find("a.rule").attr("href", $.format("/app/formrule/{0}?type=field&menuid=m21", M._id));
		c.find("a.notification").attr("href", $.format("/app/notification/{0}?type=weixin&menuid=m11", M._id));
		c.find("a.relation").attr("href", $.format("/app/fieldrelation/{0}?menuid=m32", M._id));
...
head.ready(function () {
	...
	$("#divActions").find("a.btn").add("#btnAdvanceSearch").poptip({
		position : "top"
	});
	createForm();
	createFields();
